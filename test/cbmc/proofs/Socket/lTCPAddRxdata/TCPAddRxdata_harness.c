...
StreamBuffer_t * __CPROVER_file_local_FreeRTOS_Sockets_c_prvTCPCreateStream(
    FreeRTOS_Socket_t * pxSocket,
    BaseType_t xIsInputStream );

void __CPROVER_file_local_FreeRTOS_Sockets_c_vTCPAddRxdata_Callback(
    FreeRTOS_Socket_t * pxSocket,
    const uint8_t * pcData,
    uint32_t ulByteCount );

void __CPROVER_file_local_FreeRTOS_Sockets_c_vTCPAddRxdata_Stored(
    FreeRTOS_Socket_t * pxSocket );
...
void harness()
{
    ...
    /* This function expect socket to be not NULL, as it has been validated
     * previously */
