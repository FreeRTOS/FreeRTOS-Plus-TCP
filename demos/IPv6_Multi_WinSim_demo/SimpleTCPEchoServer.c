 ...
 * to handle each connection.
 */
static void prvConnectionListeningTask6( void *pvParameters );

/*
 * Created by the connection listening task to handle a single connection.
 */
static void prvServerConnectionInstance6( void *pvParameters );
...
void vStartSimpleTCPServerTasks( uint16_t usStackSize, UBaseType_t uxPriority )
{
	...
	/* Create the TCPv6 echo server. */
	xTaskCreate( prvConnectionListeningTask6, "ServerListenerV6", usStackSize, NULL, uxPriority + 1, NULL );
...
static void prvConnectionListeningTask6( void *pvParameters )
{
struct freertos_sockaddr6 xClient, xBindAddress;
Socket_t xListeningSocket, xConnectedSocket;
socklen_t xSize = sizeof( xClient );
static const TickType_t xReceiveTimeOut = portMAX_DELAY;
const BaseType_t xBacklog = 20;
	...
	xBindAddress.sin_len = sizeof(struct freertos_sockaddr6);
	...
	for( ;; )
	{
		...
		/* Spawn a task to handle the connection. */
		xTaskCreate( prvServerConnectionInstance6, "EchoServer", usUsedStackSize, ( void * ) xConnectedSocket, tskIDLE_PRIORITY, NULL );
	}
}
/*-----------------------------------------------------------*/
