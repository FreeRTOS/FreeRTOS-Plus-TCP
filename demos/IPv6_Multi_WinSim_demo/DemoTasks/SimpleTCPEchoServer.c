...
#define tcpechoPORT_NUMBER		32002
...
//	xWinProps.lTxWinSize = configECHO_SERVER_TX_WINDOW_SIZE;  // 2
//	xWinProps.lRxBufSize = ipconfigTCP_RX_BUFFER_LENGTH;      // 100 
//	xWinProps.lRxWinSize = configECHO_SERVER_RX_WINDOW_SIZE;  // 2
...
static void prvServerConnectionInstance( void *pvParameters )
{
int32_t lBytes, lBytesLeft, lTotalSent = 0, lTotalRecv = 0;
...
uint8_t *pucRxBuffer;
	...
	FreeRTOS_printf( ("prvServerConnectionInstance: now connected\n" ) );
	...
	if( pucRxBuffer != NULL )
	{
		...
		for( ;; )
		{
			...
			if(xIsFatalError( lBytes ))
			{
				FreeRTOS_printf( ("prvServerConnectionInstance: recv returns %d\n", lBytes) );
				break;
			}
			...
			if( lBytes > 0 )
			{
				...
				while( lBytesLeft > 0 )
				{
					lBytes = FreeRTOS_send( xConnectedSocket, pucRxBuffer, lBytesLeft, 0 );
					if( xIsFatalError( lBytes ) )
					{
						break;
					}
				...
				if(xIsFatalError( lBytes ))
				{
					FreeRTOS_printf( ("prvServerConnectionInstance: send returns %d\n", lBytes) );
					break;
				}
			}
		}
...
#if 0
	/* _HT_ We get here pecause recv/send returned a fatal error. Normally an ECONN.
	 * Therefor it makes no sens to start a gracious shutdown, TCP is already shut down.
	 */
	...
	FreeRTOS_printf( ( "prvServerConnectionInstance: socket now closed  Rx/Tx = %d/%d\n",
		lTotalRecv, lTotalSent  ) );
	vTaskDelete( NULL );
}
/*-----------------------------------------------------------*/
